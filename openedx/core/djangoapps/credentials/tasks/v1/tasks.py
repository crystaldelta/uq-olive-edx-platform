...
def send_grade_if_interesting(
    ...
    if not is_learner_records_enabled_for_org(course_run_key.org):
        if verbose:
            logger.warning(f"{warning_base} the learner records feature is disabled for the {course_run_key} org")
        return

    # grab mode and/or status if we don't have them in hand, retrieve them from the learner's certificate record
